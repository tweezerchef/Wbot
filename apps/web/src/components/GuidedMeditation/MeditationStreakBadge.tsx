...
export function MeditationStreakBadge({
  ...
  useEffect(() => {
    if (showCelebration && streak > 0) {
      ...
      const t1 = setTimeout(() => { setIsAnimating(false); }, 1000);
      const t2 = setTimeout(() => {
  ...
  const containerClass = [styles.container, styles[variant], isAnimating ? styles.celebrating : ''].filter(Boolean).join(' ');
  ...
  return (
    <div className={containerClass}>
      <div className={styles.badge}>
        ...
        {variant === 'expanded' && <span className={styles.label}>day{streak !== 1 ? 's' : ''}</span>}
      </div>
