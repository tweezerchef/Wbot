...
export function BadgeUnlock({
  ...
  return (
    <div className={`${styles.overlay} ${isVisible ? styles.visible : ''} ${isClosing ? styles.closing : ''}`}>
      <div className={styles.container}>
        <div className={styles.confetti}>
          {Array.from({ length: 20 }).map((_, i) => (
            <div key={i} className={styles.particle} style={{ '--delay': `${String(i * 0.1)}s` } as React.CSSProperties} />
          ))}
        </div>
